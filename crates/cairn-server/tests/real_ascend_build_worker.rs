...
fn scheduled_exact_candidate_revision_reaches_remote_ascend_build() {
    ...
    await_candidate_build(
        ...
        ids,
        &CandidateBuildExpectation {
            publication: prepared.revision_id().to_string(),
            input_id: prepared.input_bundle_id(),
            environment_id: prepared.environment_id(),
            contract_id: prepared.contract_id(),
            expected_stdout: "PASS candidate-build=complete device=none",
...
#[test]
#[ignore = "requires the live no-device Ascend build worker and an exact archived Candidate revision"]
fn scheduled_exact_candidate_revision_reaches_product_owned_native_asc_gate() {
    let config_path = env::var("CAIRN_REAL_CONTROLLER_CONFIG").expect("controller config path");
    let revision_state = env::var("CAIRN_REAL_CANDIDATE_REVISION_STATE_DIR")
        .expect("Candidate revision episode state directory");
    let revision_id: ContentId<CollectionCandidateRevisionArtifact> =
    ...
    .expect("prepare product-owned native Candidate revision gate");
    prepared
        .archive_materials(&mut controller_content)
        .expect("archive native Candidate revision gate materials");
    let ids = schedule_ids();
    let scheduled = schedule_execution_contract(&config, prepared.contract(), ids)
        .expect("schedule native Candidate revision gate");
    assert!(matches!(
        scheduled,
        ControllerSchedulingOutcome::Scheduled { .. }
    ));
...
#[test]
#[ignore = "requires the live no-device Ascend build worker and an exact archived Candidate native follow-up"]
fn scheduled_exact_candidate_native_followup_reaches_product_owned_native_asc_gate() {
    let config_path = env::var("CAIRN_REAL_CONTROLLER_CONFIG").expect("controller config path");
    let followup_state = env::var("CAIRN_REAL_CANDIDATE_NATIVE_FOLLOWUP_STATE_DIR")
        .expect("Candidate native follow-up episode state directory");
    let followup_id: ContentId<CollectionCandidateNativeFollowupRevisionArtifact> =
        env::var("CAIRN_REAL_CANDIDATE_NATIVE_FOLLOWUP_ID")
            .expect("Candidate native follow-up ID")
            .parse()
            .expect("typed Candidate native follow-up ID");
    let image = env::var("CAIRN_REAL_ASCEND_IMAGE_ID").expect("full Ascend image ID");
    let followup_content = SqliteContentStore::open(
        Path::new(&followup_state).join("content.db"),
        Path::new(&followup_state).join("cas"),
    )
    .expect("Candidate native follow-up content store");
    let followup_bytes = read_content(&followup_content, &followup_id);
    let config = load_config(Path::new(&config_path));
    let mut controller_content = SqliteContentStore::open(
        &config.storage.content_database,
        &config.storage.content_directory,
    ...
    .expect("prepare product-owned native Candidate follow-up gate");
    prepared
        .archive_materials(&mut controller_content)
        .expect("archive native Candidate follow-up gate materials");
    let ids = schedule_ids();
    let scheduled = schedule_execution_contract(&config, prepared.contract(), ids)
        .expect("schedule native Candidate follow-up gate");
    assert!(matches!(
        scheduled,
        ControllerSchedulingOutcome::Scheduled { .. }
    ));
...
struct CandidateBuildExpectation {
    publication: String,
    input_id: ContentId<InputBundleArtifact>,
    environment_id: ContentId<ExecutionEnvironmentArtifact>,
...
fn await_candidate_build(
    ...
    job_id: JobId,
    ids: ControllerScheduleIds,
    expected: &CandidateBuildExpectation,
) {
    let job = ExecutionJob::new(job_id).expect("job stream");
    for _ in 0..1_200 {
        ...
        match recover_execution_job(&events, &content, &job).expect("recover Candidate build") {
            ExecutionJobState::Completed {
                ...
                assert_eq!(receipt.contract_id(), expected.contract_id);
                assert!(
                    evidence
                        .observations()
                        .iter()
