//! Opt-in live `DeepSeek` revision from one exact failed Candidate build receipt.
...
use cairn_migration::{
    CandidateBuildEnvironmentProfileV1, CandidateRevisionEpisodeRunInput,
    CollectionCandidateProposalArtifact, IntentRecoveryInputV1, SirReadByteLimit, SirReadLineLimit,
    ...
    run_collection_candidate_revision_episode, validate_archived_collection_candidate_proposal,
...
struct LiveCandidateRevisionConfig {
    schema_version: u16,
...
struct ResolvedLiveCandidateRevisionConfig {
...
impl LiveCandidateRevisionConfig {
    fn resolve(
        self,
        root: &Path,
    ) -> Result<ResolvedLiveCandidateRevisionConfig, Box<dyn std::error::Error>> {
        if self.schema_version != 1 {
            return Err("Candidate revision configuration must use schema_version 1".into());
        }
        ...
        Ok(ResolvedLiveCandidateRevisionConfig {
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    cairn_observability::init("candidate-revision-deepseek-live")?;
    let root = std::env::current_dir()?;
    let mut args = std::env::args().skip(1);
    let usage = "usage: candidate_revision_deepseek <task-root> <candidate-state-dir> <parent-proposal-id> <controller-config> <receipt-id> <sha256:image-id> [config-path]";
    let task_root = args.next().ok_or(usage)?;
    let candidate_state = root.join(args.next().ok_or(usage)?);
    let parent_id = args
        .next()
        .ok_or(usage)?
        .parse::<ContentId<CollectionCandidateProposalArtifact>>()?;
    ...
    let config_path = args
        .next()
        .unwrap_or_else(|| "config/candidate-revision-deepseek.example.json".to_owned());
    if args.next().is_some() {
        return Err(usage.into());
    }

    let raw: LiveCandidateRevisionConfig =
    ...
    let candidate_content = SqliteContentStore::open(
        candidate_state.join("content.db"),
        candidate_state.join("cas"),
    )?;
    let parent_bytes = read_content(&candidate_content, &parent_id)?;
    let parent = validate_archived_collection_candidate_proposal(&parent_bytes, parent_id)?;
    let search_id = parent.search_input();
    let search_bytes = read_content(&candidate_content, &search_id)?;
    let search_input =
    ...
    let recovery_bytes = read_content(&candidate_content, &recovery_id)?;
    ...
    if model.protocol().kind() != ModelProtocolKind::OpenAiResponses {
        return Err("Candidate revision requires an OpenAI Responses deployment".into());
    }
    if live.max_output_tokens > model.capabilities().max_output_tokens() {
        return Err("Candidate revision max_output_tokens exceeds model capability".into());
    }

    let workspace = SirTaskWorkspace::load(&root.join(task_root), live.task_limits)?;
    let episode_id = EpisodeId::new();
    if episode_id == parent.episode_id() {
        return Err("revision episode must be distinct from parent episode".into());
    }
    ...
    let AgentEpisodeState::Completed {
        ...
        return Err("reopened Candidate revision was not durably completed".into());
    };
    if reason != outcome.completion_reason() || steps_started != outcome.steps_started() {
        return Err("reopened Candidate revision terminal projection changed".into());
    }
    ...
    println!(
        ...
        serde_json::to_string_pretty(&serde_json::json!({
            ...
            "parent_proposal":outcome.parent_id(),
            "build_diagnostic":outcome.diagnostic_id(),
