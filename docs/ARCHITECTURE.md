...
**准入单位不是使用单位。** 一次探针运行可以同时支撑多条彼此独立的声明；把它们记成一条，
它们就再也不能被单独查询、引用、撤回或按版本限定。因此 evidence event 是准入依据，
entry 是投影、引用、撤回与适用范围的粒度，两者是多对多。

**没有读取路径的层不得有写入路径。** 一层若先落地写入，它就会积累成只写不读的材料，
而最有价值的层最容易这样死掉。任何一层的读写路径必须在同一次变更里落地。
...
（soc / arch / toolchain 约束）、逐条 entry 摘要与许可条款。**产品源码仓库不携带 knowledge，发布包携带种子 pack。**
knowledge 按 target 与厂商的时钟衰减，与源码的发布时钟不同，且第三方快照带有独立的再分发条款；
但一个装好却没有任何知识的部署无法工作，因此种子随二进制分发，而不是随源码分发。
仓库只可携带明确标注为 fixture 的测试用 pack，它不得成为事实上的产品知识。
...
- trust state 绑定 entry 修订的 content identity；字节变化产生新修订，旧裁决留在旧修订上；
- Agent 提名的 fact 与 case 由 Controller 铸造为 local pack revision，不写入 pack 目录。

**Entry 有独立于任务的稳定身份，以及自己的修订血缘。** 知识不是任务的产物而是任务的输入，
因此 entry 身份与任何 task lineage 无关：`KnowledgeEntryId` 跨修订稳定，每个修订由 content identity 标识，
并指名其 parent 修订与变更理由（更正 / 收窄适用范围 / 被取代）。修订链构成 DAG，形状与候选修订一致（见 10.6）。

由此，6.2 关于 continuation、feedback、receipt 与 hidden exposure 不得跨 lineage 投影的规则**不适用于 entry**：
那条规则约束的是关于某个任务的材料，而一条平台事实不关于任何任务。
提名它的 task 记在 provenance 里，provenance 是一个字段，从来不是投影作用域。
防止未验证知识扩散的控制是 trust state 随投影一起可见，不是限制可见范围——
一条 `Unaudited` 的 entry 带着「未验证」这一事实被投影，模型看得见。

trust state 绑定修订而不是 entry：新修订是一个新主体，旧修订的裁决留在旧修订上，
...
约束是**有序维度上的区间**，不是等值匹配。平台事实的适用范围是版本窗口——「CANN 某版起支持 X」——
等值匹配表达不了它。比较器（等值 / 属于集合 / 版本区间）是领域中立的枚举，维度名与取值由消费方提供；
知识子系统不认识 `soc` 或 `cann` 这些词，领域中立因此是类型性质而不是纪律。

**目标未解析时的行为必须是显式的。** 一道建立在未知值上的 gate 要么拦下一切要么放行一切，两者都是静默的错。
此时只有不带约束的 entry 可被投影，且该事实必须作为 typed 状态出现在投影结果中，
不得表现为「知识库是空的」。
...
| 状态 | 含义 | 谁能设置 |
| --- | --- | --- |
| `Unaudited` | 无人读过。其 supported path 可以遵循，其每一个数字都是假说 | 缺省 |
| `Reviewed` | 有人读过并给出裁决。这是一个声明，不是一次验证 | administrator |
| `Validated` | 一份存在的证据支撑它，且必须指名覆盖了哪几条 claim | 只能由机械重算 |
| `Refuted` | 一份证据否证了它。产品自有的移出供应；外部的永远与其否证一起供应 | 只能由机械重算 |

`Validated` 与 `Refuted` 是**派生量**，任何人不得直接写入:按 `AGENTS.md`，证据是 harness 在工具真正运行时记录下来的东西，
申请人填写的字段永远是 claim 而不是证据，这一条明确包含本仓库自己的工具。

**新 entry 进来即可用，并带着「未验证」这一事实一起被投影。** 一步式的准入是一个死锁——
一条程序在被遵循之前无法被验证，而在它存在之前无法被遵循——而按 3.6，死锁的门会被绕过去。
因此证伪程序不是准入前提，只是升级前提。

**撤回的证据门槛与准入对称。** 一条事实的证据是探针，因此撤回它的证据是一条与之矛盾、已经过门、自带探针的事实。
否则撤回就是一个删除键，而预算压力下的 actor 会用它删掉挡路的一切。被撤回的 entry 移出供应而不从存储中删除。

**冲突不投票。** 两条 entry 相互矛盾时标记冲突并同时投影，在 evidence dependency 足以定位前保持 unresolved（与 8 一致）。
...
#### 检索：主动查询为主，但索引必须按症状组织

主动查询是默认模式:语义检索会检回「看着对其实过期或错」的内容，而那会摧毁「验证搜索」这一立论地基。

但「让模型自己选」在它不知道自己不知道时会静默失效。实测形状是:编译器说定不了链接语言，
而需要的事实是某个语言不认某个扩展名——模型当时在看 kernel 代码，它不会去查扩展名，
因为它对问题的假说本身是错的。**主动检索要求检索者先形成正确的查询，而这正是失败的位置。**

因此 entry 可携带**可观测症状模式**，使模型手里已有的诊断本身成为索引键。两条约束：

- 症状模式只能匹配**工具链输出**（编译器/链接器消息、退出码、探针输出），不得匹配任务身份、源码内容或期望输出。
  否则它会成为把 fixture 答案送进 production knowledge 的通道。
- 症状模式是 entry 修订 content identity 的一部分，因此放宽模式会产生新修订并回到未验证，
  一条「总能命中」的模式无法沉默地保留其信任状态。

Controller 可以在症状机械匹配时注入 typed notice，形状与 6.2 的重复动作检测和预算临界通知相同；
但它**只注入「有一条已登记 entry 匹配了本次诊断」这一事实，不注入结论**。判断仍归模型。

每次投影都要说出**它没有返回什么**:沉默会被读成「其余部分是空的」。这与「安静即信任」不冲突——
前者是本次查询的覆盖面，后者是单条 entry 的信任，只在信任缺失时告警。

**截断必须保留两端并显式声明**，且不得丢弃必填的结构性字段（如 boundary）。
一次被静默截断的结果不是省下了上下文，是把它花在了更糟的地方。

#### 子系统设计的授权

本节定义该子系统的架构约束。其数据模型、生命周期状态机、crate 边界与接口形状细化于
`docs/KNOWLEDGE.md`，该文在本节授权下成为该子系统的实现依据；两者冲突以本节为准。
