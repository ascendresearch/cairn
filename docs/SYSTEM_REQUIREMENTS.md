...
| QR-SEC-006 | Before any SIR output can enter an authority decision, Semantic Intent Recovery MUST run outside the Controller and Admission authority under an exact typed profile and capability grant, without restricted-store or admitted-artifact capability. A proposal-only value spike MAY use an isolated non-authoritative harness around the existing agent runtime, provided its output cannot reach an admitted consumer. A different data/tool/OS capability boundary requires a different process instance, and the later mechanical Admission gate remains model-free. | Proposal-only reachability test first; process/dependency, capability, and OS-permission tests when authority integration is introduced. |
