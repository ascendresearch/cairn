...
| FR-EXEC-026 | Legacy static certificate bindings MUST migrate atomically into the persistent credential registry without changing `WorkerId`, `CredentialId`, pool, or certificate fingerprint. Import MUST retain an auditable provenance fact, use an explicit strong `CommandId`, replay the exact command idempotently, and reject credential, fingerprint, worker-ownership, or command-input collisions. Ordinary controller startup and scheduling MUST consume only registry authority and MUST reject legacy static entries through an explicit configuration-version gate. | Batch import/replay and collision unit tests; contradictory-history projection control; two-worker V2 import followed by empty-static V3 mTLS authentication, resource refresh, scheduling, and restart projection. |
