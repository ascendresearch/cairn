...
| FR-EXEC-032 | Replicated input and environment artifacts MUST use strict versioned canonical formats before a real executor interprets them. Sandbox paths MUST remain strong relative values; V1 tree expansion MUST support only explicit directories and regular files, use private create-only per-attempt paths, reject symlinks/special files/missing parents, and never overwrite a prior attempt tree. Worker bootstrap MUST keep execution disabled; activation MUST validate execution mode, exact advertised backend, concurrency, and ready availability together. A local-process adapter MUST initially require concurrency and availability one, clear ambient environment, use argv without a shell, fail closed when its required network isolation cannot be established, supervise on a blocking task while the control session remains live, enforce contract timeout and capture/output bounds, serialize terminal journal publication after any intervening delivery facts, and preserve the existing NotStarted/Ambiguous/restart authority semantics. It MUST NOT be represented as hostile-code filesystem isolation until that boundary is actually enforced. | Canonical material positive/negative suite; create-only replay control; strict disabled/activated configuration tests; real supervised success and process-group timeout tests; unsupported-network pre-start rejection; result publication after an intervening acknowledgement fact; documentation/security-boundary review. |
| FR-EXEC-033 | The CPU-only OCI backend MUST accept only an immutable SHA-256 image digest and a strict canonical OCI environment. Its container name MUST derive from the exact `AttemptId`; present runtime phases MUST carry a full runtime container ID and immutable labels binding attempt, job, contract, input, environment, and code-owned sandbox policy. Mutable tags, short IDs, arbitrary names, impossible phase/identity combinations, conflicting bindings, and provider-native inspect output outside a `ContainerRuntime` adapter MUST fail closed. Until the non-weakenable launch plan, recoverable supervisor, bounded capture/evidence path, concrete runtime preflight, and real-host isolation gates are all implemented, a worker MUST NOT advertise or activate this backend. | Strong-type and strict-decode tests; canonical environment identity mutation suite; fake-runtime resolution/inspection fixture; worker activation rejection until the complete backend exists. |
| FR-EXEC-034 | The CPU-only OCI launch plan MUST derive only from the canonical contract, worker-verified exact material, a safe backend-owned state root, and positive strong resource ceilings. It MUST render shell-free deterministic argv with immutable local image resolution, explicit replacement of any image entrypoint by the contract program, a read-only root and single read-only input bind, separate bounded subject-owned work/output/temporary tmpfs mounts, a fixed numeric non-root user, disabled network, all capabilities dropped, `no-new-privileges`, private runtime namespaces, and hard PID/CPU/memory-and-swap ceilings. Backend, identity, material, layout, device, network, or minimum-resource mismatch MUST fail before runtime authority exists. Operators MUST NOT be able to add mounts, devices, capabilities, namespace sharing, network, or mutable image pulls. A launch plan alone MUST NOT activate the backend or imply that image/runtime/user-namespace preflight and real-host isolation have passed. | Exact golden argv; repeat-render determinism; mount/privilege negative inspection; identity/material/layout/network/device/resource negative tests; execution configuration continues to reject OCI activation. |
| FR-EXEC-035 | The OCI runtime capabilities MUST expose only typed image resolution, deterministic-name inspection, create-with-the-code-owned-plan, start-by-full-runtime-ID, and bounded terminal supervision for the same ID. Image-declared volumes MUST fail before create. The sole initial supervisor entry MUST inspect before mutation and MAY return `NotStarted` only while no container exists and no mutation may have applied. An unknown create/start response and every failed recovery MUST be `Ambiguous`. Recovery MUST validate the exact name, full runtime ID, and complete attempt/job/contract/input/environment/policy binding before acting; it MAY create an absent deterministic name or reissue start for the same `Created` container, but MUST NOT start `Exited`, mutate/delete a conflict, or create a second runtime subject. A definitive create race MUST be re-inspected before classification. | Typed runtime/exit round trip; fake-runtime matrix for every phase, response loss before/after create and start effect, concurrent create winner, changed name/runtime/binding, declared image volume, completion during disconnect, exited replay, exact mutation counts, and continued worker activation rejection. |
| FR-EXEC-036 | OCI terminal supervision MUST apply the contract timeout to total runtime-observed execution time across reconnects and drain stdout/stderr independently under their exact bounds. Timeout or stream exhaustion MUST request stop only for the matching full runtime ID and reconcile by inspection even when the stop response is lost. Capture MUST begin only from a matching `Exited` state, retain at most bounded stream prefixes, ingest only declared regular files below individual and aggregate plan bounds, and classify missing, symlink/special/directory, or oversized output as integrity failure. Trusted evidence MUST be bounded and obtained outside candidate-writable mounts, including observed immutable image, resolved program, full runtime ID, fixed policy, elapsed time, termination reason, and exit state. Lifecycle/capture authority MUST contain no cleanup operation; separate cleanup authority MUST remain unavailable until the exact terminal worker observation is durable, and cleanup failure MUST NOT authorize re-execution. | Fake-runtime stop ambiguity before/after effect, deadline-preserving recovery, independent stream and retained-prefix bounds, missing/non-regular/oversized output, terminal image/binding conflict, evidence bound, disconnect completion, stable terminal replay, capability-boundary review, and continued worker activation rejection. |
