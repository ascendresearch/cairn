...
| FR-EXEC-013 | Workers MUST initiate the control connection. V1 deployment transport MUST use mutually authenticated TLS and binary canonical-JSON WebSocket messages. The controller MUST bind the verified leaf-certificate fingerprint to the exact enrolled `WorkerId` before registration; TLS identity alone MUST NOT create a domain fact. | mTLS loopback test checking both server-name and client-certificate verification, fingerprint equality, strict first-message hello, and mismatched enrollment rejection. |
