...
| FR-INTENT-013 | The first Intent Authority Proof MUST use a Cairn-authored clean-room CUDA one-dimensional binary32 sum with an explicit host launch. Its first admitted domain is contiguous normal-or-signed-zero `f32`, `1 <= N <= 256`, `abs(x_i) <= 65536`, non-aliasing read-only input, and single-element overwritten output. Mathematical intent is real-number sum; arbitrary permutation/parenthesization with round-to-nearest-ties-to-even binary32 addition is calibration evidence, signed zeros are numerically equivalent, and exact source-tree bits are not semantic authority. Launch geometry/reduction tree are implementation artifacts. Empty input, subnormals, non-finite values, wider shapes, source anomalies, and caller/source conflicts remain explicitly outside or pending user decision. | Source/provenance manifest, public honest/tail/order-sensitive/wrong-hypothesis/unknown controls, six-partition non-adaptive sealed corpus, and user-decision fixtures. |
...
| FR-ORACLE-032 | The first Intent proof MUST use the deterministic mechanism set fixed by D-040: strict decode/identity validation, ABI fact extraction, required-evidence derivation, deterministic recipe/plan validation, recorded/host execution, observation comparison, receipt binding/closure, intent policy evaluation, Mechanical Gate/admitted construction, and diagnostic redaction. Every member MUST be independently qualified before Gate use; a model-backed Admission Planner is outside this first set. | Exact mechanism inventory and identities; independent golden/property expectations; targeted mutation, wrong-binding, missing/duplicate/tamper, runner fault/restart, redaction-canary, real-host calibration, independent review, and requalification controls. |
