...
| FR-ORACLE-020 | Oracle synthesis and adversarial exploration MUST be distinct typed workflow strategies when both are used. They run as Controller workflow steps and MUST NOT consume one another's private model continuation, unsubmitted reasoning, or unrecorded mutable context. Policy MAY use non-agent mutation/property/counterexample strategies instead of a model-backed adversarial step. | Workflow binding audit, attempted private-history cross-link rejection, and a non-agent adversarial strategy fixture. |
...
| FR-AGENT-006 | Agent roles and strategy steps MUST have distinct typed inputs, continuation bindings, tool catalogs, results, and budgets; prompt text alone is not a boundary. These steps execute in the Controller workflow and MUST NOT acquire process-local authority. Any operation requiring a different execution capability MUST be represented as a Worker job rather than moving the Agent step into another process. | Synthesis/adversarial/planning/candidate binding matrix, cross-step substitution controls, and request→capability-matched-Worker tests. |
...
| FR-AGENT-022 | Cairn MUST distinguish Agent-capable function, strategy, planner/agent profile, durable model execution, Controller workflow step, and authority in product types and records. The current profile catalog and its derived count MUST NOT be encoded as a fixed process, concurrency, protocol, or release constant; a required product function MAY use an admitted deterministic strategy when policy permits. | Catalog derivation test, typed identity compile-fail tests, deterministic strategy substitution, and no hard-coded profile-count architecture check. |
| FR-AGENT-023 | Cross-step Agent interaction MUST occur only through immutable, provenance-bearing artifacts, typed requests/diagnostics, and durable events selected by trusted policy. A step MUST NOT read another step's private continuation, unpublished reasoning, pending tool results, or unsubmitted drafts; Agent agreement, voting, or repeated reflection MUST NOT increase evidence strength or replace authoritative receipts. | Cross-step binding tests, artifact-edge reconstruction, private-context denial controls, and agreement-without-receipt negative test. |

The product Agent catalog, invocation policy, step interaction, model-runtime sharing, and Worker capability routing
...
| QR-SEC-006 | Before any SIR output can enter an authority decision, SIR MUST run as an exact typed step of the Controller workflow and its output MUST remain proposal-only. It MUST NOT receive restricted-store material, admitted constructors, Worker receipts, or policy/user authority. Any compilation, applicant execution, Docker, host adapter, or device request MUST become a Controller-authorized `JobContract` executed by a capability-matched managed Worker; model output cannot manufacture the resulting observation. The later mechanical Admission gate remains model-free. | Workflow reachability, proposal/admitted static boundaries, request→job→Worker observation lineage controls, and no-direct-execution dependency scan. |
