...
| FR-ORACLE-021 | Blue MUST have a bounded read-only external-test research tool. Search/fetch requests, immutable upstream revision when available, exact fetched bytes or bounded excerpt, provenance, truncation, and license evidence or explicit unknown MUST remain reconstructable. | Recorded PyTorch-like search/fetch control plus query/source/bytes/license identity mutations. |
| FR-ORACLE-022 | External or upstream tests MUST remain proposals and MUST NOT become trusted cases by origin. Search snippets alone MUST NOT become executable corpus cases, and unknown/incompatible license provenance MUST prevent copied bytes from entering distributable fixtures. | Trust-promotion and unknown-license negative controls. |
