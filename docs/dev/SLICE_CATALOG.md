...
| `DEV-020` | In progress | 将DEV-019 exact repair publication原样送回同一product-owned native ASC gate | DEV-019；[`DEV-020 DCR`](records/DEV-020-IMPLEMENTATION.md) | exact repair envelope/source/primary进入existing Controller与remote `npu-build` Worker；取得可恢复terminal receipt；不自动repair、不含semantic/verdict |
