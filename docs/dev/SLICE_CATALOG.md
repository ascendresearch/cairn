...
| `DEV-021` | Proposed | 将DEV-008–020已观察到的transition固化为Controller-owned、可重放的current-V1 workflow spine，先关闭Candidate native build/diagnostic/repair的手工串接 | D-043；DEV-020；[`下一会话交接`](NEXT_SESSION.md) | entry DCR需明确task-generic strong states、recorded consumer、restart/illegal-transition controls和superseded orchestration删除项；用户确认后才能转`Ready`并实施；不以新manual repair、空Proposal Host或完整workflow大爆炸替代该proof |
