...
| `DEV-022` | Accepted | 以同一generic Proposal Host承载SIR与Candidate领域角色，并消费DEV-021 persisted episode request | D-043；DEV-021；[`DEV-022 DCR`](records/DEV-022-IMPLEMENTATION.md) | SIR/Candidate cross-role controls、workflow round-trip、strict V1/binding negatives、real child SQLite restart terminal replay、专用SIR/Candidate launcher删除闭合；无live model/Worker/evidence claim |
| `DEV-023` | Accepted | 让active Controller以单任务process manager消费durable Candidate next action，并监管generic Host与existing Worker scheduler | D-043；DEV-022；[`DEV-023 DCR`](records/DEV-023-IMPLEMENTATION.md) | exact Host binary/start marker、prepare-before-effect、scheduler/reconcile/receipt折回、two-task/NoCandidate exact replay、旧public helper接缝删除闭合；无live model/Worker/NPU claim |
| `DEV-024` | Accepted | 删除role-specific runner，固化一个generic Proposal Host request/episode/observation/submission/terminal lifecycle | D-043；DEV-022；DEV-023；[`DEV-024 DCR`](records/DEV-024-IMPLEMENTATION.md) | common-loop external-effect fail-closed、duplicate capability、invalid-submit repair、SIR/Candidate isolation、workflow round-trip与旧runner symbol scan；无live model/Worker/NPU claim |
...
| `DEV-033` | Accepted | 将task-generic Candidate authority、Proposal Host、product-owned build与独立Admission并入同一个Controller aggregate | D-043；DEV-030–032；[`DEV-033 record`](records/DEV-033-IMPLEMENTATION.md) | admitted-only authority、exact Oracle bodies、generic Host、durable build start/receipt、机械plane controls、model-free terminal闭合；旧collection/native Candidate suffix已删除，无live effect claim |
