...
- 实施状态：本目录主要描述目标架构；DEV-008已实现第一条窄的SIR/Intent Admission进程与authority
  boundary，DEV-009–012只推进到一个host-side local Oracle claim的observation、qualification、publication、
  Candidate search input和首个unbuilt source proposal；其他组件是否存在仍以开发基线为准
