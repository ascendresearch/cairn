...
| 候选合约与台账用两种规范序 | 整个 Oracle 阶段付完之后 | 已在构造处排序，见 4.9 |
...
### 4.9 Oracle 阶段首次完整通过，候选入口另有一堵墙

**首次走完:** portfolio 提案接受 → 机制资格认证 → 五个控制族全部执行 →
80 次 worker 控制作业全部 `Succeeded` → Oracle Admission 完成重算。随后进入候选阶段时失败于
`ContractDrift`。

**根因:同一批数据存在两种规范序。** 台账按 dimension 的语义序（plane、concern、role）排列
obligation，因为那是阅读顺序;`CandidateOracleContractV1` 以 content id 为键，其 `validate`
要求按 content id 严格递增。`derive` 继承台账序再以另一种序校验，15 个 dimension 同时满足两种序
只能靠哈希巧合，因此真实 portfolio 必然过不了自己的检查。已在构造处排成检查要读的序。

**既有测试调用过 `derive` 且通过——因为它只有一条 entry，而单元素集合在任何序下都有序。**
新测试用生产同样的 15 个 concern，并直接断言后置条件而不是只看调用成功。
红验证去掉排序后复现出线上相同的 `ContractDrift`。
...
而它存在的唯一原因是当初归档位置选错了。现在一个任务的全部材料是一个目录,结束就是删目录。

冻结源码从不是构建输入（构建只取 Controller 配方与候选自己提交的文件），
所以 10.6 那条「构建输入永远不是工作区」不适用于它。
搬过来还有一个副作用:普通目录自身没有完整性保证,于是「按冻结 bundle 重建」从一道摆设
变成唯一挡住文件被就地改写的检查——红验证这次真的触发了,搬家之前它触发不了。
