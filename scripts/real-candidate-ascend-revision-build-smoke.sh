...
if [[ $# -ne 4 ]]; then
  echo "usage: scripts/real-candidate-ascend-revision-build-smoke.sh <controller.json> <sha256:image-id> <candidate-revision-state-dir> <candidate-revision-id>" >&2
  exit 2
fi
...
CAIRN_REAL_CANDIDATE_REVISION_ID="$REVISION_ID" \
  cargo test -p cairn-server --test real_ascend_build_worker \
    scheduled_exact_candidate_revision_reaches_remote_ascend_build -- --ignored --nocapture
